...
export function resolveOptions(options) {
    const resolved = cloneOptions(defaultOptions);
    if (!options) {
        return resolved;
    }
    ...
    if (typeof options.sampleCount === 'number')
        resolved.sampleCount = Math.max(16, Math.floor(options.sampleCount));
    ...
    if (typeof options.initialValue === 'number')
        resolved.initialValue = options.initialValue;
    if (options.smoothing)
        resolved.smoothing = { ...resolved.smoothing, ...options.smoothing };
    if (options.futureMask)
        resolved.futureMask = { ...resolved.futureMask, ...options.futureMask };
    if (options.progressBar)
        resolved.progressBar = { ...resolved.progressBar, ...options.progressBar };
    if (typeof options.containerBackground === 'string')
        resolved.containerBackground = options.containerBackground;
    return resolved;
}
export function cloneOptions(source) {
    return {
        width: source.width,
...
function assignLineOptions(target, source) {
    if (!source)
        return;
    ...
    if (source.dashPattern)
        target.dashPattern = source.dashPattern.slice();
}
export function lineStyleToDash(style, custom) {
...
export function maskOpacity(kind, baseOpacity) {
