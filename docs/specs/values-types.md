...
1.7 **Vectors of numbers are first-class signal data.** They lower to fixed-size data tables and are consumed by `step`, `gates`, `seq`, `interp`, `for`, etc. A vector containing time-varying expressions (e.g. `[1 (sin beat) 3]`) is also valid; each slot is its own signal.
...
resolvable because they affect lowering or state-slot identity. For example,
UGens may use `:phase`, `:id`, and `:fresh` to control initial phase and
state-sharing ([state.md §6.9](state.md)).
