...
1.2 **Cells are reactive bindings** in Reactive mode. When the user redefines a cell, every output whose compiled graph references that cell is invalidated and recompiled with the new value.
...
1.5 **Dependency tracking is by symbol identity, not by source text.** Renaming a cell does not preserve dependencies; it is logically a new cell.

1.6 **Diamond / cascading dependencies** propagate transitively. `(define a 1) (define b (+ a 1)) (define c (+ b 1)) (a1 (* c beat))` — redefining `a` invalidates the entire chain to `a1`.
...
1.9 A cell's body in Reactive mode is itself a (possibly trivial) signal expression. `(define sweep (+ 200 (* 200 t)))` creates a time-varying cell. References to `sweep` from output expressions inline its body.
