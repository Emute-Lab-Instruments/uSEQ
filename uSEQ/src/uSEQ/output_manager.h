...
public:
    ...
    Value handle_output_setter(int output_id, OutputType type,
                               std::vector<Value>& args, Environment& env);
    Value handle_output_getter(int output_id, OutputType type,
                               std::vector<Value>& args, Environment& env);
...
private:
    ...
    // Helper functions
    Value set_continuous_output(int output_id, std::vector<Value>& args,
                                Environment& env);
    Value set_binary_output(int output_id, std::vector<Value>& args,
                            Environment& env);
    Value set_serial_output(int output_id, std::vector<Value>& args,
                            Environment& env);

    Value get_continuous_output(int output_id, std::vector<Value>& args,
                                Environment& env);
    Value get_binary_output(int output_id, std::vector<Value>& args,
                            Environment& env);
    Value get_serial_output(int output_id, std::vector<Value>& args,
                            Environment& env);
};
...
{
public:
    static Value setter_function(uSEQ* useq, std::vector<Value>& args,
                                 Environment& env);
    static Value getter_function(uSEQ* useq, std::vector<Value>& args,
                                 Environment& env);
};
...
#define USEQ_OUTPUT_SETTER(id, type)                                                \
    ...
    {                                                                               \
        return OutputFunctionGenerator<                                             \
            id, OutputManager::OutputType::type>::setter_function(this, args, env); \
    }
...
#define USEQ_OUTPUT_GETTER(id, type)                                                \
    ...
    {                                                                               \
        return OutputFunctionGenerator<                                             \
            id, OutputManager::OutputType::type>::getter_function(this, args, env); \
    }
