...
int generateChipAddress()
{
    ...
    // Serial.println((hex2int(chipID[strlen(chipID)-2]) * 16) +
    // hex2int(chipID[strlen(chipID)-1]));
    int id = (hex2int(chipID[strlen(chipID) - 2]) * 16) +
             hex2int(chipID[strlen(chipID) - 1]);
