...
int aOutExpanderAddr[] = {0,0,0,0,0}; //array for any analog out expanders - future we could load this array from FFS as currently the order is also the channel number i.e. entry +10 A11 is output 1 on aOutExpander[0]
...
void i2cWriteString(int expander, String msg){
  ...
  while (packetStart < msg.length()){
    ...
    if ((msg.length() - packetStart) > (packetSize-1)){
      i2cHOST->write(msg.substring(packetStart, packetStart+packetSize).c_str(),  packetSize);
    } else {
      i2cHOST->write(msg.substring(packetStart).c_str(), msg.length()-packetStart+1);
    }
    i2cHOST->endTransmission();
...
String getI2CResults(int expander){
 ...
 // Serial.println(c, DEC);
  ...
  int maxPacketLen = 17;
  while( nRecdChars < c){
    if((c-nRecdChars)>maxPacketLen) nCharRequested = maxPacketLen;
    else nCharRequested = c-nRecdChars;
...
void syncAllI2c(){
  for (int i = 0; i<nOutExpander; i++ ) i2cWriteString(i, "$sync");
}
