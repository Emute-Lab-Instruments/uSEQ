...
const OutputManager::OutputConfig* OutputManager::get_config(int output_id,
                                                             OutputType type)
{
...
bool OutputManager::is_output_available(int output_id, OutputType type)
{
    ...
    case OutputType::CONTINUOUS:
        return output_id >= 1 &&
               static_cast<size_t>(output_id) <= NUM_CONTINUOUS_OUTS;
...
Value OutputManager::handle_output_setter(int output_id, OutputType type,
...
Value OutputManager::handle_output_getter(int output_id, OutputType type,
                                          std::vector<Value>& args, Environment& env)
{
    // Validate output is available for current hardware
    if (!is_output_available(output_id, type))
...
Value OutputManager::set_continuous_output(int output_id, std::vector<Value>& args,
                                           Environment& env)
{
...
Value OutputManager::set_binary_output(int output_id, std::vector<Value>& args,
                                       Environment& env)
{
...
Value OutputManager::set_serial_output(int output_id, std::vector<Value>& args,
                                       Environment& env)
{
...
Value OutputManager::get_continuous_output(int output_id, std::vector<Value>& args,
                                           Environment& env)
{
    const auto* config = get_config(output_id, OutputType::CONTINUOUS);
    if (!config)
...
Value OutputManager::get_binary_output(int output_id, std::vector<Value>& args,
                                       Environment& env)
{
    const auto* config = get_config(output_id, OutputType::BINARY);
    if (!config)
...
Value OutputManager::get_serial_output(int output_id, std::vector<Value>& args,
                                       Environment& env)
{
    const auto* config = get_config(output_id, OutputType::SERIAL_OUT);
    if (!config)
...
// Template method implementations (moved from header to avoid incomplete type
// warnings)
template <int OutputId, OutputManager::OutputType Type>
Value OutputFunctionGenerator<OutputId, Type>::setter_function(
    uSEQ* useq, std::vector<Value>& args, Environment& env)
{
    return useq->get_output_manager().handle_output_setter(OutputId, Type, args,
                                                           env);
}

template <int OutputId, OutputManager::OutputType Type>
Value OutputFunctionGenerator<OutputId, Type>::getter_function(
    uSEQ* useq, std::vector<Value>& args, Environment& env)
{
    return useq->get_output_manager().handle_output_getter(OutputId, Type, args,
                                                           env);
}
