...
void ErrorManager::report_undefined_symbol(const String& symbol_name, const String& code, int line, int column) {
    ErrorContext& ctx = ensure_error_context(ErrorCategory::UNDEFINED_SYMBOL, 
        "Variable '" + symbol_name + "' is not defined");
    
    ctx.symbol_name = symbol_name;
    ctx.detailed_message = "The symbol '" + symbol_name + "' was not found in the current environment.";
    ctx.suggestion = "Define the variable first using (def " + symbol_name + " value) or check for typos.";
    ctx.line = line;
    ctx.column = column;
...
void ErrorManager::report_undefined_function(const String& function_name, const String& code, int line, int column) {
    ErrorContext& ctx = ensure_error_context(ErrorCategory::UNDEFINED_FUNCTION,
    ...
    ctx.suggestion = "Check the function name spelling or ensure the function is defined.";
    ctx.line = line;
    ctx.column = column;
    ...
    if (function_name == "sni") {
        ...
        add_did_you_mean("sin");
        ctx.suggestion = "Did you mean 'sin'? Note: function names are case-sensitive";
    }
}

// Report arity error (wrong number of arguments)
void ErrorManager::report_arity_error(const String& function_name, int expected, int received) {
    ...
    if (function_name == "sin") {
        ...
        add_example("(+ 1 2)");
        add_example("(+ 1 2 3 4)");
        ctx.suggestion = "Usage: (+ number1 number2 ...)\\nExample: (+ 1 2 3) returns 6";
    }
}

// Report type error
void ErrorManager::report_type_error(const String& function_name, const String& expected_type, 
    ...
    if (function_name == "sin" && received_type.indexOf("string") >= 0) {
        ctx.suggestion = "The sin function calculates the sine of a phase value (0.0 to 1.0)\\nExample: (sin 0.25) returns 1.0 (peak of sine wave)";
    }
}

// Report arithmetic error
void ErrorManager::report_arithmetic_error(const String& message) {
    ErrorContext& ctx = ensure_error_context(ErrorCategory::ARITHMETIC_ERROR, message);
    
    if (message.indexOf("division") >= 0 && message.indexOf("zero") >= 0) {
        ctx.suggestion = "Division by zero is undefined. Check your denominator values.";
    }
}

// Report syntax error
void ErrorManager::report_syntax_error(const String& message, const String& code, int line, int column) {
...
ErrorContext& ErrorManager::ensure_error_context(ErrorCategory category, const String& message) {
    if (!current_error.has_value()) {
