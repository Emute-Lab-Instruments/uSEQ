...
void FunctionRegistry::registerFunction(
    const String& name, const String& module,
...
Value* FunctionRegistry::lookup(const String& name)
{
    ...
    if (it != functions_.end())
    {
        ...
        if (desc_it != descriptors_.end())
        {
            ...
            {
                // Return if module is enabled or if function has no module (core
                // functions)
                return &it->second;
            }
        }
...
std::vector<String> FunctionRegistry::getFunctionNames() const
{
    ...
    {
        if (lookup(pair.first))
        { // Only include enabled functions
            names.push_back(pair.first);
        }
    }
    return names;
...
std::vector<String>
FunctionRegistry::getFunctionNamesByModule(const String& module) const
{
...
{
    // Since we can't store lambdas in Value directly, we need to handle this
    // differently For now, we'll store the function pointer directly in the
    // descriptor and create a simple wrapper function
