...
public:
    ...
    uSEQDSPEngine()
    {
        ...
        add_repeating_timer_ms(
            ...
            {
                return static_cast<uSEQDSPEngine*>(rt->user_data)
    ...
    bool FAST_FUNC(command_timer_callback)()
    {
        ...
        while (queue_try_remove(&DSPQ::q_engine_commands, &cmd))
        {
            ...
            case CREATE:
                ...
                break;
            case CONNECT:
                connect(cmd.data.connect.srcKey, cmd.data.connect.channelSrc,
                        cmd.data.connect.destKey, cmd.data.connect.channelDest);
                break;
            case DISCONNECT:
            ...
            case MESSAGE:
            {
                ...
                {
                    it->second->message(String(cmd.data.message.message),
                                        cmd.data.message.value);
                }
                else
                {
                    println("Processor not found: " +
                            String(cmd.data.message.ugen_key));
                }
            }
            break;
            default:
    ...
    void FAST_FUNC(connect)(size_t srcKey, size_t channelSrc, size_t destKey,
                            size_t channelDest)
    {
        ...
        {
            println("Processor(s) not found: " + String(srcKey) + " or " +
                    String(destKey));
        }
    }
    ...
    bool FAST_FUNC(run)(double sampleRate)
    {
        ...
        add_repeating_timer_us(
            -quantum, [](repeating_timer_t* rt) -> bool
    ...
    void FAST_FUNC(request_ugen_info)()
    {
        for (size_t i = 0; i < uGenFactories.size(); i++)
        {
            ...
            println("UGENINFO: " + String(resp.data.ugenInfo.key) + " " +
                    factory.name);
        }
    }
...
private:
    ...
    {
        uGenFactories.push_back({ ugenName, [](size_t key) -> componentPtr
                                  {
                                      return std::make_shared<ugenType>(
                                          &DSPQ::q_engine_responses, key);
                                  } });
    };
