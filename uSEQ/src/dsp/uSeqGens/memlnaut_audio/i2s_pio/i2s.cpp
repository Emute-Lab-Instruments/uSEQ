 ...
 *
 * rp2040_i2s_example is free software: you can redistribute it and/or modify it
...
static void calc_clocks(const i2s_config* config, pio_i2s_clocks* clocks)
{
    ...
    float sck_desired = (float)config->fs * (float)config->sck_mult *
                        (float)i2s_sck_program_pio_mult;
    float sck_attained = pio_div(sck_desired, &clocks->sck_d, &clocks->sck_f);
    clocks->fs_attained =
        sck_attained / (float)config->sck_mult / (float)i2s_sck_program_pio_mult;
    ...
    clocks->sck_pio_hz = pio_div(sck_hz * (float)i2s_sck_program_pio_mult,
                                 &clocks->sck_d, &clocks->sck_f);
    float bck_hz       = clocks->fs_attained * (float)config->bit_depth * 2.0f;
    clocks->bck_pio_hz = pio_div(bck_hz * (float)i2s_out_master_program_pio_mult,
                                 &clocks->bck_d, &clocks->bck_f);
}
...
static void dma_double_buffer_init(pio_i2s* i2s, void (*dma_handler)(void))
{
    ...
    channel_config_set_ring(&c, false, 3);
    channel_config_set_transfer_data_size(&c, DMA_SIZE_32);
    dma_channel_configure(i2s->dma_ch_in_ctrl, &c,
                          &dma_hw->ch[i2s->dma_ch_in_data].al2_write_addr_trig,
                          i2s->in_ctrl_blocks, 1, false);
    ...
    dma_channel_configure(i2s->dma_ch_in_data, &c,
...
static void i2s_slave_program_init(PIO pio, const i2s_config* config, pio_i2s* i2s)
{
    ...
    i2s_bidi_slave_program_init(pio, i2s->sm_din, offset, config->dout_pin,
                                config->din_pin);
...
static void i2s_sync_program_init(PIO pio, const i2s_config* config, pio_i2s* i2s)
{
    ...
    if (config->sck_enable)
    {
        ...
        if (!validate_sck_bck_sync(&clocks))
        {
             ...
             *  - You are running a system clock frequency that doesn't divide well
             * at all into SCK or BCK
             *  - You are running a 24-bit I2S with a 256x SCK multiplier (RP2040
             * cannot support this)
    ...
    i2s_out_master_program_init(pio, i2s->sm_dout, offset, config->bit_depth,
                                config->dout_pin, config->clock_pin_base);
    pio_sm_set_clkdiv_int_frac(pio, i2s->sm_dout, clocks.bck_d, clocks.bck_f);
}

void i2s_program_start_slaved(PIO pio, const i2s_config* config,
                              void (*dma_handler)(void), pio_i2s* i2s)
{
...
void i2s_program_start_synched(PIO pio, const i2s_config* config,
                               void (*dma_handler)(void), pio_i2s* i2s)
{
