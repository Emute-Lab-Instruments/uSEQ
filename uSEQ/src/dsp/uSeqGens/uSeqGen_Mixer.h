...
{
public:
    uSeqGen_Mixer(queue_t* q, size_t key, size_t numInputs = 2)
        : uSeqGen_Base(q, key)
    {
        inputCount = 2;
        ...
        addMessageHandler("set_inputs",
                          [this](command_data_message_data& data)
                          {
                              const size_t nInputs =
                                  static_cast<size_t>(data.floatData.value);
                              if (nInputs >= 1)
                              {
    ...
    {
        return (static_cast<size_t>(inputIndex) < muteStates.size())
                   ? muteStates[inputIndex]
                   : true;
    }
