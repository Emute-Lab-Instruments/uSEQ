...
public:
    uSeqGen_NN(queue_t* q, size_t key) : uSeqGen_Base(q, key)
    {
        ...
        const std::vector<ACTIVATION_FUNCTIONS> layers_activfuncs = { RELU, RELU,
                                                                      RELU,
                                                                      SIGMOID };
        const bool use_constant_weight_init                       = false;
        const float constant_weight_init                          = 0;
        // Layer size definitions
        const std::vector<size_t> layers_nodes = { n_inputs + kBias, 10, 10, 14,
                                                   n_outputs };

        // Create MLP
        mlp = std::make_unique<MLP<float>>(layers_nodes, layers_activfuncs,
                                           loss::LOSS_MSE, use_constant_weight_init,
        ...
        addMessageHandler("rand", [this](command_data_message_data& data)
                          { mlp->DrawWeights(); });
        ...
        //     MLP<float>::training_pair_t ds(dataset->GetFeatures(),
        //     dataset->GetLabels());
