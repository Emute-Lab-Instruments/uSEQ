...
TEST_CASE("Storage Environment - Basic save and load", "[storage][environment]")
{
    ...
    SECTION("Save and load simple variables")
    {
        ...
        useq_instance.__test_get_defs()[String("y")] =
    ...
    SECTION("Save and load expressions")
    {
        ...
        Value lambda_expr = Value({ Value::atom(String("lambda")),
                                    Value({ Value::atom(String("x")) }),
                                    Value({ Value::atom(String("+")),
                                            Value::atom(String("x")), Value(1) }) });

        Value addition_expr =
            Value({ Value::atom(String("+")), Value(10), Value(20) });
    ...
    SECTION("Save and load mixed definitions")
    {
        ...
        useq_instance.__test_get_defs()[String("name")] =
            Value::string(String("test_module"));

        // Add expression definitions
        useq_instance.__test_get_def_exprs()[String("doubler")] = Value(
            { Value::atom(String("lambda")), Value({ Value::atom(String("n")) }),
              Value({ Value::atom(String("*")), Value::atom(String("n")),
                      Value(2) }) });
        ...
        REQUIRE(useq_instance.__test_get_defs()[String("name")].as_string() ==
                String("test_module"));

        auto doubler_it =
            useq_instance.__test_get_def_exprs().find(String("doubler"));
...
TEST_CASE("Storage Environment - Edge cases", "[storage][edge_cases]")
{
    ...
    SECTION("Large environment")
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            useq_instance.__test_get_def_exprs()[expr_name] =
                Value({ Value::atom(String("+")), Value(i), Value(i + 1) });
        }
