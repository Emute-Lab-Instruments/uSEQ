...
TEST_CASE("Static string evaluation with environment", "[interpreter][api][static_eval]") {
    Environment env;
...
TEST_CASE("Static value evaluation with environment", "[interpreter][api][static_eval][values]") {
    Environment env;
...
TEST_CASE("Argument evaluation with expressions", "[interpreter][api][eval_args][expressions]") {
    Environment env;
    env.set("x", Value(5));
    env.set("y", Value(3));
...
TEST_CASE("Complex expressions - conditionals and functions", "[interpreter][api][complex_expressions]") {
    ...
    REQUIRE_FALSE(result1.length() == 0);
    if (result1.indexOf("error") == -1) {
        REQUIRE((result1.indexOf("true-case") != -1 || result1.indexOf("false-case") != -1));
    }
