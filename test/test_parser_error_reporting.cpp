...
TEST_CASE("Parser reports unexpected end of input for incomplete expressions", "[parser][error][syntax]") {
    ErrorManager error_mgr;
    uLispParser parser(&error_mgr);
    ...
    SECTION("Incomplete list") {
        ...
        REQUIRE(error != nullptr);
        REQUIRE(error->category == ErrorCategory::SYNTAX_ERROR);
        REQUIRE(error->primary_message == "Malformed program - unexpected end of input");
    }
...
TEST_CASE("Parser reports unterminated string literals", "[parser][error][syntax]") {
    ...
    SECTION("Missing closing quote") {
        ...
        REQUIRE(error != nullptr);
        REQUIRE(error->category == ErrorCategory::SYNTAX_ERROR);
        REQUIRE(error->primary_message == "Unexpected end of input, expected closing quote");
    }
...
TEST_CASE("Parser handles complex nested error scenarios", "[parser][error][complex]") {
    ErrorManager error_mgr;
    uLispParser parser(&error_mgr);
...
TEST_CASE("Parser works correctly with no ErrorManager", "[parser][error][compatibility]") {
    // Test backward compatibility - parser should not crash with nullptr ErrorManager
...
TEST_CASE("Parser error reporting preserves error context", "[parser][error][context]") {
    ErrorManager error_mgr;
    uLispParser parser(&error_mgr);
...
TEST_CASE("Parser error messages are descriptive and helpful", "[parser][error][usability]") {
    ErrorManager error_mgr;
    uLispParser parser(&error_mgr);
