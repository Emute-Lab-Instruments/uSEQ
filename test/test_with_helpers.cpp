...
void testBasicEvaluation()
{
    ...
    std::cout << "DEBUG: Expected '3', got '" << result
              << "' (length=" << result.length() << ")" << std::endl;
    for (size_t i = 0; i < result.length(); i++)
    {
        std::cout << "  [" << i << "]: '" << result[i] << "' (ascii "
...
int main()
{
    ...
    std::cout
        << "This test demonstrates faster compilation by using minimal interfaces."
        << std::endl;
    ...
    try
    {
        ...
        std::cout << "\n\u2713 All tests using minimal interfaces passed!"
                  << std::endl;
        std::cout << "The test compiled faster by avoiding heavy headers."
                  << std::endl;
        return 0;
    }
