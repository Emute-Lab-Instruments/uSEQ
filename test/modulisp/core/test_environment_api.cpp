...
TEST_CASE("Environment construction and basic initialization",
...
TEST_CASE("Expression and value namespace separation",
          "[environment][expressions][separation]")
{
    Environment env;
...
TEST_CASE("Variable shadowing in environment scopes",
          "[environment][scoping][shadowing]")
{
    Environment parent_env;
    Environment child_env;
    child_env.set_parent_scope(&parent_env);
...
TEST_CASE("Global variable setting in environment hierarchy",
...
TEST_CASE("Global expression setting in environment hierarchy",
          "[environment][global][expressions]")
{
    Environment parent_env;
    Environment child_env;
    child_env.set_parent_scope(&parent_env);
...
TEST_CASE("Environment combination with conflicts",
          "[environment][combination][conflicts]")
{
    Environment env1;
    Environment env2;
    ...
    SECTION("conflicting variable handled")
    {
        ...
        // The exact behavior depends on implementation, but value should be one of
        // the two
        REQUIRE(
            (conflict_result->as_int() == 100 || conflict_result->as_int() == 200));
    }
