...
TEST_CASE("Phasor offset function", "[modulisp][api][useq_phasor_offset]")
{
    ...
    REQUIRE(result2.as_float() ==
            Approx(0.3).epsilon(0.001)); // fmod(1.3, 1.0) = 0.3
...
TEST_CASE("Euclidean rhythm function", "[modulisp][api][useq_euclidean]")
{
    ...
    std::vector<Value> args1 = { Value(8), Value(3), Value(0), Value(0.5),
                                 Value(0.0) };
    ...
    std::vector<Value> args2 = { Value(8), Value(3), Value(0), Value(0.5),
                                 Value(0.5) };
...
// NOTE: Gates function commented out as it requires evaluation context for the
// pattern TEST_CASE(test_useq_gates) {
//     ModuLispInterpreter interp;
//     Environment env;
//
...
TEST_CASE("Scheduling functions", "[modulisp][api][scheduling]")
{
    ...
    std::vector<Value> schedule_args = { Value::string("test_item"), Value(1.0),
                                         Value(42) };
