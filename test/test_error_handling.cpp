...
TEST_CASE("Syntax errors with helpful messages", "[error-handling][syntax]") {
    ...
    SECTION("Unbalanced parentheses - missing closing") {
        ...
        REQUIRE(response.get_suggestion() == "Add ')' at the end of your expression");
        REQUIRE(response.get_context_snippet() == "(defn make-beat [phase] (sin (* phase 2)\n                                        ^-- Expected ')' here");
    }
    ...
    SECTION("Unclosed string literal") {
        ...
        REQUIRE(response.get_suggestion() == "Add a closing '\"' to complete the string");
        REQUIRE(response.get_context_snippet().find("\"Hello live coders") != std::string::npos);
    }
    ...
    SECTION("Invalid character in symbol") {
        ...
        REQUIRE(response.is_error());
        REQUIRE(response.get_error_type() == ModuLisp::ErrorType::SYNTAX_ERROR);
        REQUIRE(response.get_error_message() == "Invalid character '(' in symbol name");
        REQUIRE(response.get_line() == 1);
        REQUIRE(response.get_column() == 12);
        REQUIRE(response.get_suggestion() == "Symbol names can only contain letters, numbers, hyphens, and underscores");
    }
}

TEST_CASE("Undefined function errors with suggestions", "[error-handling][undefined]") {
    ModuLisp modulisp;
    ...
    SECTION("Typo in function name - close match") {
        ...
        REQUIRE(response.get_suggestion() == "Did you mean 'sin'? (sine wave function)");
    }
    ...
    SECTION("Common misspelling - oscillator") {
        ...
        REQUIRE(response.is_error());
        auto suggestions = response.get_did_you_mean();
        REQUIRE(std::find(suggestions.begin(), suggestions.end(), "oscillator") != suggestions.end());
        REQUIRE(response.get_suggestion() == "Did you mean 'oscillator'? (generates audio waveforms)");
    }
    ...
    SECTION("Wrong case sensitivity") {
        ...
        REQUIRE_FALSE(response.okay());
        REQUIRE(response.is_error());
        REQUIRE(response.get_suggestion() == "Did you mean 'sin'? Note: function names are case-sensitive");
    }
    ...
    SECTION("No close matches") {
        ...
        REQUIRE_FALSE(response.okay());
        REQUIRE(response.is_error());
        REQUIRE(response.get_error_message() == "Function 'xyzabc123' is not defined");
        REQUIRE(response.get_did_you_mean().empty());
        REQUIRE(response.get_suggestion() == "No similar functions found. Type (help) to see available functions");
    }
}
...
TEST_CASE("Argument errors with helpful explanations", "[error-handling][arguments]") {
    ...
    SECTION("Wrong number of arguments - too few") {
        auto response = modulisp.send("(from-list [1 2 3])");  // + needs at least 2 args
        REQUIRE_FALSE(response.okay());
        REQUIRE(response.is_error());
        REQUIRE(response.get_error_type() == ModuLisp::ErrorType::ARITY_ERROR);
        REQUIRE(response.get_error_message() == "Function 'from-list' expects exactly 2 arguments, but got 1");
        REQUIRE(response.get_suggestion() == "Usage: (from-list vector phasor)\nExample: (from-list [1 2 3] bar)");
    }
    ...
    SECTION("Wrong number of arguments - too many") {
        ...
        REQUIRE(response.is_error());
        REQUIRE(response.get_error_type() == ModuLisp::ErrorType::ARITY_ERROR);
        REQUIRE(response.get_error_message() == "Function 'sin' expects exactly 1 argument, but got 2");
        REQUIRE(response.get_suggestion() == "Usage: (sin phase)\nExample: (sin 0.5) returns sine wave value at phase 0.5");
    }
    
    SECTION("defn with missing parameters") {
        auto response = modulisp.send("(defn make-beat)");  // Missing param list and body
        REQUIRE_FALSE(response.okay());
        REQUIRE(response.is_error());
        REQUIRE(response.get_error_type() == ModuLisp::ErrorType::ARITY_ERROR);
        REQUIRE(response.get_error_message() == "defn requires a name, parameter list, and body");
        REQUIRE(response.get_suggestion() == "Usage: (defn function-name [param1 param2 ...] body-expression)\nExample: (defn double [x] (* x 2))");
    }
    ...
    SECTION("Wrong argument type") {
        ...
        REQUIRE(response.get_error_message() == "Function 'sin' expects argument 1 to be a number, but got string \"hello\"");
        REQUIRE(response.get_suggestion() == "The sin function calculates the sine of a phase value (0.0 to 1.0)\nExample: (sin 0.25) returns 1.0 (peak of sine wave)");
    }
}
...
TEST_CASE("Runtime errors with context", "[error-handling][runtime]") {
    ...
    SECTION("List index out of bounds") {
        ...
        REQUIRE(response.get_error_message() == "Index 5 is out of bounds for list of size 4");
        REQUIRE(response.get_suggestion() == "Valid indices are 0 to 3. Use (count notes) to check list size.");
    }
    ...
    SECTION("Stack overflow from infinite recursion") {
        ...
        REQUIRE(response.get_error_message() == "Maximum recursion depth exceeded (stack overflow)");
        REQUIRE(response.get_suggestion() == "Your function is calling itself infinitely. Add a base case to stop recursion.\nExample: (defn countdown [n] (if (<= n 0) \"done\" (countdown (- n 1))))");
    ...
    SECTION("Timeout in long-running computation") {
        auto response = modulisp.send("(reduce + (range 1000000))");  // Very long computation
        // This would need timeout detection in the interpreter
        if (response.is_error() && response.get_error_type() == ModuLisp::ErrorType::TIMEOUT_ERROR) {
            REQUIRE(response.get_error_message() == "Evaluation timed out after 100ms");
            REQUIRE(response.get_suggestion() == "Your code is taking too long. Consider using smaller values or optimizing the algorithm.");
        }
    }
}
...
TEST_CASE("Variable and binding errors", "[error-handling][variables]") {
    ...
    SECTION("Undefined variable") {
        ...
        REQUIRE(response.is_error());
        REQUIRE(response.get_error_type() == ModuLisp::ErrorType::UNDEFINED_VARIABLE);
        REQUIRE(response.get_error_message() == "Variable 'x' is not defined");
        REQUIRE(response.get_suggestion() == "Define the variable first using (def x value) or use it as a function parameter");
    }
    ...
    SECTION("Typo in variable name with suggestion") {
        ...
        REQUIRE(response.is_error());
        REQUIRE(response.get_error_type() == ModuLisp::ErrorType::UNDEFINED_VARIABLE);
        REQUIRE(response.get_error_message() == "Variable 'amplitued' is not defined");
        auto suggestions = response.get_did_you_mean();
        REQUIRE(std::find(suggestions.begin(), suggestions.end(), "amplitude") != suggestions.end());
    ...
    SECTION("Attempting to redefine built-in") {
        ...
        REQUIRE(response.get_error_message() == "Cannot redefine built-in function 'sin'");
        REQUIRE(response.get_suggestion() == "Choose a different name for your variable. Built-in functions are protected.");
    }
}
...
TEST_CASE("Pattern matching and destructuring errors", "[error-handling][patterns]") {
    ...
    //     auto response = modulisp.send("(let [[a b c] [1 2]] (+ a b c))");  // Not enough values
    //     REQUIRE(response.is_error());
    //     REQUIRE(response.get_error_type() == ModuLisp::ErrorType::DESTRUCTURING_ERROR);
    //     REQUIRE(response.get_error_message() == "Cannot destructure: expected 3 values but got 2");
    //     REQUIRE(response.get_suggestion() == "The pattern [a b c] expects exactly 3 values.\nEither provide 3 values or adjust the pattern to match.");
    // }
    ...
    SECTION("Invalid binding form") {
        ...
        REQUIRE(response.is_error());
        REQUIRE(response.get_error_type() == ModuLisp::ErrorType::SYNTAX_ERROR);
        REQUIRE(response.get_error_message() == "let requires an even number of forms in binding vector");
        REQUIRE(response.get_suggestion() == "Usage: (let [name1 value1 name2 value2 ...] body)\nExample: (let [x 10 y 20] (+ x y))");
    }
}
...
//         REQUIRE(context.find("5:            (+ root interval))") != std::string::npos);
//         REQUIRE(context.find("6:          scle)))  ; Typo here") != std::string::npos);
//         REQUIRE(context.find("          ^^^^") != std::string::npos);  // Error marker
//     }
// }
...
//         REQUIRE(response.get_suggestion().find("Did you mean to use a string?") != std::string::npos);
//         REQUIRE(response.get_suggestion().find("(println \"Hello World\")") != std::string::npos);
//     }
    
//     SECTION("Using = instead of ==") {
//         auto response = modulisp.send("(if (= x 5) true false)");  // Clojure uses = for equality
//         // This should actually work in Clojure-like syntax, but if not implemented:
//         if (response.is_error()) {
//             REQUIRE(response.get_suggestion().find("For equality comparison, use =") != std::string::npos);
//         }
//     }
    
//     SECTION("Forgot to quote a list") {
//         auto response = modulisp.send("(def notes (60 62 64))");  // Should be [60 62 64] or '(60 62 64)
//         REQUIRE(response.is_error());
//         REQUIRE(response.get_error_type() == ModuLisp::ErrorType::UNDEFINED_FUNCTION);
//         REQUIRE(response.get_error_message() == "Function '60' is not defined");
//         REQUIRE(response.get_suggestion().find("use square brackets [60 62 64]") != std::string::npos);
//     }
// }
...
//         REQUIRE(response.get_warnings()[0] == "Function 'set-tempo' is deprecated. Use 'set-bpm' instead.");
//     }
    
//     SECTION("Performance warning") {
//         auto response = modulisp.send("(map sin (range 10000))");  // Large computation
//         REQUIRE(response.okay());
//         REQUIRE(response.get_warnings().size() > 0);
//         REQUIRE(response.get_warnings()[0].find("Large computation") != std::string::npos);
//     }
...
//         REQUIRE(response.get_warnings().size() == 1);
//         REQUIRE(response.get_warnings()[0] == "Variable 'y' is defined but never used");
//     }
// }
...
//         auto response = modulisp.send("(oscillator 440 0.5 \"sine\" true false)");  // Way too many args
        
//         // Simple message for beginners
//         REQUIRE(response.get_error_message() == "Too many arguments for oscillator");
...
//         REQUIRE(std::find(suggestions.begin(), suggestions.end(), "osc-bank") != suggestions.end());
//     }
    
//     SECTION("Pattern context suggestions") {
//         modulisp.set_context(ModuLisp::Context::PATTERN);
//         auto response = modulisp.send("(seq [1 2 3])");  // Might be looking for sequence functions
//         if (response.is_error()) {
//             auto suggestions = response.get_did_you_mean();
//             REQUIRE(std::find(suggestions.begin(), suggestions.end(), "sequence") != suggestions.end());
//             REQUIRE(std::find(suggestions.begin(), suggestions.end(), "step-seq") != suggestions.end());
//         }
//     }
// }

// TEST_CASE("Error formatting for different outputs", "[error-handling][formatting]") {
//     ModuLisp modulisp;
...
//         REQUIRE(terminal_output.find("Variable 'two' is not defined") != std::string::npos);
//     }
...
//         REQUIRE(json.find("\"message\": \"Variable 'two' is not defined\"") != std::string::npos);
//         REQUIRE(json.find("\"suggestions\": [") != std::string::npos);
//     }
